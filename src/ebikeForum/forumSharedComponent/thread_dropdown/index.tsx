...
function Thread_dropdown({ setSelectedVaie }: { setSelectedVaie: (value: string) => void }) {
  ...
  const handleSubcategory = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setSelectedVaie(event.target.value);
  };
          ...
          {selectedCategory ?
            (data
              ...
              ?.sub_category.map((e: any, i: number) => (
                <option key={i} value={e?.name}   className={styles.subcatgegory_options}>
                  {e?.name}
                </option>
              ))) :
